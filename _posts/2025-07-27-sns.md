...
Now that we have CloudTrail logging all of our AWS events, we need to set up a system that alerts us of security concerns. First, we must set up an SNS topic. **Simple Notification Service** allows publishers to send messages to subscribers. When enabled with CloudWatch, when certain events are identified (such as DeleteGuardrail), AWS will send a message straight to your email.
...
3. Configure the Topic Settings
   ...
   - Name: Enter a unique name like security-alerts-topic or <&lt;your initial>-email to indicate the communication channel.
