...
**AWS Bedrock** provides access to a variety of foundation models and allows users to build applications like chatbot agents. Bedrock supports <a href="https://aws.amazon.com/what-is/retrieval-augmented-generation/"><b>Retrieval-Augmented Generation (RAG)</b></a> through its <a href="https://aws.amazon.com/bedrock/knowledge-bases/"><b>Knowledge Bases (KB)</b></a> feature. This allows the chatbot to provide more personalized and context-aware responses by retrieving relevant information from a connected KB. These knowledge bases typically contain proprietary company information that the model can reference to improve the accuracy of its responses.
...
- Click on **Available to request** for Claude 3 Haiku.

Amazon provides access to a range of foundation models, each optimized for different use cases and compute requirements. Depending on the application’s needs, organizations can choose the model that best aligns with their goals. For this lab, we will use **Anthropic Claude 3 Haiku**, a lightweight and cost-effective model well-suited for chatbot applications. In this lab, we will focus on Claude 3 Haiku. However, you are welcome to request access to all Anthropic models for experimentation. Access is currently free for testing purposes.
...
We’ve created a few files for the Knowledge Base. Click the button to download the files and explore the contents inside. Upload these two files to an S3 Bucket.
...
- **Step 3:** Configure data storage and processing
  ...
  - Choose Aurora PostgreSQL as the vector store.
    ...
    \*Note: Any vector store can be used. We chose Aurora PostgreSQL for this lab because it charges per query, which can be more cost-effective than time-based billing.
...
Creating an agent connects the KB to the model. Agents securely connect your company’s data sources and user inputs to the KB and RAG system to generate accurate, contextual responses.
...
- Under **Agent details**
  ...
  - Provide instructions for the agent
    - Example: Your task is to assist clients with their questions about the bank. Be polite in your responses and always offer additional help or clarification if anything is unclear.
...
<details>
<summary>Now test out the agent! There should be a <b>Test Agent</b> window to the right. Select the agent you want to test and click "Prepare". Now, attempt to obtain sensitive information (ex. SSN, address, balance). What data were you able to reveal?</summary>
<br>
<p>
Since we haven't implemented any security mechanisms yet, the agent may expose personal information when prompted. To mitigate this, we will implement Bedrock guardrails in the next steps.</p>
</details>
